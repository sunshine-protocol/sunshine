...
fn get_last_event() -> RawEvent<u64, u32, u64, u64, u64> {
    System::events()
        ...
        .map(|r| r.event)
        .filter_map(|e| {
            if let TestEvent::bounty3(inner) = e {
                Some(inner)
            } else {
                None
            }
...
fn contribution_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Bounty3::contribute_to_bounty(Origin::signed(2), 2, 5),
            Error::<Test>::BountyDNE
        );
        assert_noop!(
            Bounty3::contribute_to_bounty(Origin::signed(2), 1, 4),
            Error::<Test>::ContributionMustExceedModuleMin
        );
        assert_noop!(
            Bounty3::contribute_to_bounty(Origin::signed(2), 1, 99),
            sp_runtime::DispatchError::Module {
                index: 0,
                error: 3,
                message: Some("InsufficientBalance",),
        ...
        assert_ok!(Bounty3::contribute_to_bounty(Origin::signed(2), 1, 5));
        assert_eq!(
            RawEvent::BountyRaiseContribution(2, 5, 1, 15),
            get_last_event()
        );
    });
}
...
fn submission_works() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            Bounty3::submit_for_bounty(Origin::signed(2), 1, 10u32, 15u64,),
            Error::<Test>::BountyDNE
        );
        assert_ok!(Bounty3::post_bounty(
            Origin::signed(1),
        ...
        assert_noop!(
            Bounty3::submit_for_bounty(Origin::signed(1), 1, 10u32, 15u64,),
            Error::<Test>::DepositerCannotSubmitForBounty
        );
        assert_noop!(
            Bounty3::submit_for_bounty(Origin::signed(2), 1, 10u32, 22u64,),
            Error::<Test>::BountySubmissionExceedsTotalAvailableFunding,
        );
        assert_ok!(Bounty3::submit_for_bounty(
            Origin::signed(2),
            1,
            10u32,
            10u64,
        ...
        assert_eq!(
            RawEvent::BountySubmissionPosted(2, 1, 10, 1, 10),
            get_last_event()
        );
    });
}
...
fn submission_approval_works() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            Bounty3::approve_bounty_submission(Origin::signed(1), 1),
        ...
        assert_noop!(
            Bounty3::approve_bounty_submission(Origin::signed(2), 1),
        ...
        assert_ok!(Bounty3::approve_bounty_submission(Origin::signed(1), 1));
