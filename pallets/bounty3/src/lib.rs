...
decl_event!(
        ...
        Balance = BalanceOf<T>,
    {
        ...
        BountyRaiseContribution(AccountId, Balance, BountyId, Balance),
        BountySubmissionPosted(AccountId, BountyId, Balance, SubmissionId, IpfsReference),
        BountyPaymentExecuted(BountyId, Balance, SubmissionId, Balance, AccountId, IpfsReference),
    }
);
...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        fn contribute_to_bounty(
            ...
            Self::deposit_event(RawEvent::BountyRaiseContribution(contributor, new_amount, bounty_id, total));
            Ok(())
        }
        #[weight = 0]
        fn submit_for_bounty(
            ...
            Self::deposit_event(RawEvent::BountySubmissionPosted(submitter, bounty_id, amount, id, submission_ref));
            Ok(())
        }
        #[weight = 0]
        fn approve_bounty_submission(
            ...
            let new_total = new_bounty.total();
            // submission approved and executed => can be removed
            <Submissions<T>>::remove(submission_id);
            <Bounties<T>>::insert(bounty_id, new_bounty);
            Self::deposit_event(RawEvent::BountyPaymentExecuted(bounty_id, new_total, submission_id, submission.amount(), submission.submitter(), submission.submission()));
            Ok(())
        }
    }
}
