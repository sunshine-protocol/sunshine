...
/// The binary vote signal type
pub type Signal<T> = <<T as Trait>::BinaryVoteMachine as OpenVote<
    OrgId<T>,
    ShareId<T>,
...
pub trait Trait: frame_system::Trait {
    ...
    type ShareData: GroupMembership<Self::AccountId>
        ...
        + IDIsAvailable<OrgItemPrefixKey<OrgId<Self>, ShareId<Self>>>
        + GenerateUniqueID<OrgItemPrefixKey<OrgId<Self>, ShareId<Self>>>
    ...
    type BinaryVoteMachine: GetVoteOutcome<OrgId<Self>, ShareId<Self>>
        + OpenVote<OrgId<Self>, ShareId<Self>, Self::AccountId, Self::BlockNumber, Permill>
        + VoteOnProposal<OrgId<Self>, ShareId<Self>, Self::AccountId, Self::BlockNumber, Permill>
        + IDIsAvailable<OrgTwoItemPrefixKey<OrgId<Self>, ShareId<Self>, VoteId<Self>>>
        + GenerateUniqueID<OrgTwoItemPrefixKey<OrgId<Self>, ShareId<Self>, VoteId<Self>>>;
...
decl_storage! {
    trait Store for Module<T: Trait> as Bank {
        ...
        DefaultThresholdForShareIdProposalType get(fn default_threshold_for_share_id_proposal_type):
            double_map hasher(opaque_blake2_256) OrgItemPrefixKey<OrgId<T>, ShareId<T>>, hasher(opaque_blake2_256) ProposalType
            => Option<ThresholdConfig<Signal<T>, Permill>>;
        ...
        ProposalDefaultVoteSchedule get(fn proposal_default_vote_schedule_for_organization):
            double_map hasher(opaque_blake2_256) OrgId<T>, hasher(opaque_blake2_256) ProposalType => Option<Vec<ScheduledVote<ShareId<T>, Signal<T>, Permill>>>;

        /// TODO: replace with VoteSchedule for codomain
        LiveVoteSequences get(fn live_vote_sequences):
            double_map hasher(opaque_blake2_256) OrgId<T>, hasher(opaque_blake2_256) ProposalIndex => Option<VoteSchedule<ShareId<T>, VoteId<T>, Signal<T>, Permill>>;
...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        fn register_organization(
            ...
            // TODO: should be an option, requiring a proposed_id is determinedly bad
            proposed_id: OrgId<T>,
            ...
            let authentication: bool = <<T as Trait>::ShareData as SudoKeyManagement<
                <T as frame_system::Trait>::AccountId,
            >>::is_sudo_key(&summoner);
            // only the sudo key can register new organizations
            ensure!(authentication, Error::<T>::OnlySudoKeyCanSummonForNow);
            ...
            let _ = <<T as Trait>::ShareData as OrgSupervisorKeyManagement<
                OrgId<T>,
                <T as frame_system::Trait>::AccountId,
            >>::swap_supervisor(new_org_id, summoner.clone(), d_supervisor)?;
        ...
        fn update_value_constitution(
            ...
            let authentication: bool = <<T as Trait>::ShareData as OrgSupervisorKeyManagement<
                OrgId<T>,
                <T as frame_system::Trait>::AccountId,
            >>::is_organization_supervisor(organization, &author) ||
        ...
        fn register_shares_in_organization(
            ...
            let new_organization = old_organization.add_new_inner_share_group(new_share_id);
        ...
        fn create_single_vote_for_existing_share_group_in_organization(
            ...
            custom_threshold: Option<ThresholdConfig<Signal<T>, Permill>>,
        ) -> DispatchResult {
            // ensure that there is an existing organization (invariants elsewhere must ensure that this check syncs with all storage item changes)
            ensure!(!Self::id_is_available(organization), Error::<T>::NoRegisteredOrganizationWithIDProvided);
            let author = ensure_signed(origin)?;
            ...
            let new_vote_id = <<T as Trait>::BinaryVoteMachine as OpenVote<
                ...
                Permill,
            >>::open_vote(
                // for now, default share weighted but this should be changed
                organization, share_id, None, SupportedVoteTypes::ShareWeighted.into(), threshold.into(), None,
            )?;
        ...
        fn set_organization_share_id_proposal_type_default_threshold(
            ...
            share_id: ShareId<T>,
            proposal_type: ProposalType,
            // TODO: change this to take ThresholdType<Signal<T>, Permill>
            passage_threshold_pct: Permill,
            turnout_threshold_pct: Permill,
            // form it from those
            // trait on associated type should be able to check consistency for a given threshold
            // the threshold also has a new method for forming it from these two parameters
        ) -> DispatchResult {
            ...
            let authentication: bool = <<T as Trait>::ShareData as OrgSupervisorKeyManagement<
                OrgId<T>,
                <T as frame_system::Trait>::AccountId,
            >>::is_organization_supervisor(organization, &setter) ||
            <<T as Trait>::ShareData as SudoKeyManagement<
                <T as frame_system::Trait>::AccountId,
            >>::is_sudo_key(&setter);
            ensure!(authentication, Error::<T>::NotAuthorizedToSetOrgShareIdProposalTypeThreshold);
            // TODO: check that both threshold parameters are of the same type
            // TODO: add vote type to this configuration point such that we can set it based on the vote type as well
            let threshold = ThresholdConfig::new_percentage_threshold(passage_threshold_pct, turnout_threshold_pct);
...
impl<T: Trait> SetDefaultShareIdThreshold<OrgId<T>, ShareId<T>> for Module<T> {
    type ThresholdConfig = ThresholdConfig<Signal<T>, Permill>;
...
impl<T: Trait> VoteScheduleBuilder<OrgId<T>, ShareId<T>> for Module<T> {
    type ScheduledVote = ScheduledVote<ShareId<T>, Signal<T>, Permill>;
...
impl<T: Trait> VoteScheduler<OrgId<T>, ShareId<T>, VoteId<T>> for Module<T> {
    type VoteSchedule = VoteSchedule<ShareId<T>, VoteId<T>, Signal<T>, Permill>;
    ...
    fn dispatch_vote_schedule_from_vec_of_share_id(
        ...
        let mut next_ordered_scheduled_votes: Vec<ScheduledVote<ShareId<T>, Signal<T>, Permill>> =
            Vec::new();
            ...
            if let Ok(new_vote_to_schedule) = Self::scheduled_vote_from_share_id_proposal_type(
                ...
                *share,
                ...
                if votes_left_including_current == 0 {
                    ...
                    let prefix_key = OrgItemPrefixKey::new(organization, *share);
                    let threshold =
                        <DefaultThresholdForShareIdProposalType<T>>::get(prefix_key, proposal_type)
                            .ok_or(Error::<T>::DefaultThresholdForShareIdNotSet)?;
                    let new_vote_id = <<T as Trait>::BinaryVoteMachine as OpenVote<
    ...
    fn move_to_next_scheduled_vote(
        ...
        let (current_share_id, current_vote_id) =
            if let Some(vote_left) = mutable_schedule.clone().pop() {
...
impl<T: Trait> ScheduleVoteSequence<OrgId<T>, ShareId<T>, VoteId<T>> for Module<T> {
...
impl<T: Trait> PollActiveProposal<OrgId<T>, ShareId<T>, VoteId<T>> for Module<T> {
